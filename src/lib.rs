...
#[no_mangle]
pub fn run() {
    listen_to_dm("DarumaDocker", handler);
}
...
fn handler(tm: TextMessage) {
    if !tm.author.bot {
        ...
        match r.len() {
            0 => create_text_message_in_dm("DarumaDocker", String::from("No images"), Some(tm.id)),
            _ => create_text_message_in_dm("DarumaDocker", r[0].clone(), Some(tm.id)),
        }
    }
}
